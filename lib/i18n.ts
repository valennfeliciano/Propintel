...
const en: Dict = {
  ...
  "market.freshness.title": "Are these numbers live?",
  "market.freshness.body": "Not yet, by design. These are a snapshot pulled from FRED on the dates shown above. They refresh when the data pipeline re-runs, not on every page view, which is fine because the series move slowly: the 30-year mortgage rate publishes weekly, the Fed funds rate monthly, and the Austin home-price index only quarterly. (A weekly auto-refresh on deploy can make it truly live.)",
...
const es: Dict = {
  ...
  "market.freshness.title": "¿Estos números son en vivo?",
  "market.freshness.body": "Todavía no, a propósito. Son una instantánea tomada de FRED en las fechas mostradas arriba. Se refrescan cuando el proceso de datos se vuelve a ejecutar, no en cada visita, lo cual está bien porque las series cambian lento: la tasa hipotecaria a 30 años se publica cada semana, la tasa de la Fed cada mes, y el índice de precios de Austin solo cada trimestre. (Un refresco automático semanal al desplegar puede hacerlo realmente en vivo.)",
