...
// ---------------------------------------------------------------------------
// Static generation — pre-build all 50 property pages.
// ---------------------------------------------------------------------------
...
// Revalidate weekly (604 800 s) — property listings don't change daily, but we
// want fresh FRED rates to keep the math current without hammering the API.
...
export async function generateMetadata({
  ...
  const description =
    `${property.beds}bd/${property.baths}ba ${property.propertyType.toLowerCase()} in ${property.neighborhood}, Austin TX. ` +
    `Listed at ${usd(property.price)}. Cap rate ${m.capRatePct}%, est. cash flow ${usdCompact(m.estMonthlyCashFlow)}/mo. ` +
...
export default async function PropertyPage({
  ...
  // Similar properties: same neighborhood, excluding self, up to 3.
  ...
  if (property.lotSqft > 0)
    facts.push(["Lot", `${property.lotSqft.toLocaleString()} sqft`]);
  ...
  const realEstateJsonLd = {
    ...
    additionalProperty: [
      ...
      {
        "@type": "PropertyValue",
        name: "Estimated Monthly Cash Flow",
        value: usdCompact(m.estMonthlyCashFlow),
      },
      {
        "@type": "PropertyValue",
        name: "Price per Sqft",
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: `${BASE_URL}/?neighborhood=${encodeURIComponent(property.neighborhood)}`,
      },
      {
        "@type": "ListItem",
        position: 3,
      ...
      <div className="flex min-h-screen flex-col bg-white">
        ...
        <header className="sticky top-0 z-10 border-b border-slate-100 bg-white/95 backdrop-blur">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-5 py-3">
            ...
            >
              ...
              <span aria-hidden>/</span>
              <span className="max-w-[220px] truncate text-slate-900 font-medium">
                {property.address}
              </span>
            </nav>
        ...
        <div className="mx-auto w-full max-w-6xl px-5 pt-6">
          ...
          <div className="mt-1 flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="font-mono text-2xl font-bold text-slate-900 sm:text-3xl">
              {usd(property.price)}
            </div>
          </div>
        ...
        <div className="mx-auto grid w-full max-w-6xl gap-8 px-5 py-8 lg:grid-cols-3">
          ...
          <div className="space-y-7 lg:col-span-2">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                ...
                <Metric
                  label="Rent/price"
                  value={`${m.rentToPricePct}%`}
                  good={m.rentToPricePct >= 0.6}
                />
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              {analysis.highlights.length > 0 && (
                <div>
                  <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-emerald-600">
                    Highlights
                  </h2>
            ...
            {property.description && (
              <div>
                ...
                <p className="mt-2 text-[11px] text-slate-400">
                  Listing prose is scraped verbatim from Zillow and may not
            ...
            <div className="space-y-1 border-t border-slate-100 pt-4">
              ...
              <p className="max-w-[72ch] text-[11px] leading-relaxed text-slate-400">
                Analysis is algorithmic and for informational purposes only. It
          ...
          {/* RIGHT — sticky deal snapshot */}
          <aside className="lg:col-span-1">
            <div className="space-y-4 lg:sticky lg:top-20">
              <div className="rounded-2xl border border-slate-200 p-5 shadow-sm">
                {/* Verdict pill */}
                <div
                  className={`flex items-center justify-between rounded-xl px-3 py-2.5 ring-1 ${rec.bg} ${rec.ring}`}
                >
                  <span
                    className={`flex items-center gap-2 text-base font-bold ${rec.text}`}
                  >
                    <span
                      className={`h-2.5 w-2.5 rounded-full ${rec.dot}`}
                    />
                ...
                <div className="mt-4 border-t border-slate-100 pt-3">
                  {property.contact?.agent ? (
                    <p className="text-sm text-slate-700">
                      ...
                      {property.contact.agentPhone && (
                        <>
                          {" · "}
                          <a
                            href={`tel:${property.contact.agentPhone}`}
                    ...
                    <p className="text-xs leading-relaxed text-slate-500">
                      Contact details available on the Zillow listing page.
                    </p>
                  )}
                </div>
              </div>
          ...
          <section className="border-t border-slate-100 bg-slate-50/50 px-5 py-10">
            <div className="mx-auto max-w-6xl">
              ...
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {similar.map((p) => {
                  const simAnalysis = analyzeProperty(p, market.mortgage30.value);
                  const simRec = recTone(simAnalysis.recommendation);
                  const ppsf = p.price / p.sqft;
                  const deltaPct =
                    ((ppsf - p.neighborhoodAvgPricePerSqft) /
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-40 w-full overflow-hidden bg-slate-100">
                        ...
                        />
                        <div
                          className={`absolute left-3 top-3 rounded-full px-2 py-0.5 text-xs font-semibold ring-1 ${simRec.bg} ${simRec.ring} ${simRec.text}`}
                        >
                      ...
                      <div className="flex flex-1 flex-col gap-3 p-4">
                        <div>
                          <div className="flex items-baseline justify-between gap-2">
                            ...
                            >
                              {below ? "▼" : "▲"}{" "}
                              {Math.abs(deltaPct).toFixed(0)}% vs comps
                            </span>
                          </div>
                        ...
                        <span className="mt-auto text-xs font-medium text-emerald-700 group-hover:underline">
                          View analysis →
                        </span>
                      </div>
                    </Link>
        ...
        <footer className="border-t border-slate-100 bg-slate-50 px-5 py-6 text-center text-xs text-slate-400">
          <p>
            &copy; {new Date().getFullYear()} PropIntel · Data sourced from
            Zillow and FRED · For informational purposes only
          </p>
        </footer>
      </div>
    </>
...
// ---------------------------------------------------------------------------
// Share button — needs client interactivity, extracted as a small island
// ---------------------------------------------------------------------------

// Inline so we don't need a separate file for a single button.
// The "use client" directive scopes only this component.
import ShareButtonClient from "@/components/ShareButton";

// Re-export as a server-renderable wrapper that passes static props.
function ShareButton({ url, address }: { url: string; address: string }) {
  return <ShareButtonClient url={url} address={address} />;
}
